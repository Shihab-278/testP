...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    } else if ($receiver['balance'] < $amount) {
        $error = 'Not enough balance to withdraw!';
    } else {
        // Deduct the amount from receiver's balance
        $stmt = $conn->prepare("UPDATE users SET balance = balance - ? WHERE id = ?");
        $stmt->execute([$amount, $receiver_id]);

        // Log the withdrawal
        $stmt = $conn->prepare("INSERT INTO credit_transfers (sender_id, receiver_id, amount, transfer_type) VALUES (?, ?, ?, 'withdrawal')");
...
<div class="content-wrapper">
    ...
    <section class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-6">
                <h4><i class="fa fa-caret-right fw-r5"></i> Withdraw Credit</h4>
                </div>
                <div class="col-sm-6">
...
<!-- Add this script to show the modal if withdrawal is successful -->
<?php if ($success): ?>
