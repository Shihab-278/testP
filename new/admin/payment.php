...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($receiver_id) {
        // Ensure the admin has enough balance to transfer
        $stmt = $conn->prepare("SELECT balance FROM users WHERE id = ?");
        $stmt->execute([$user_id]);
        $sender = $stmt->fetch();
        $sender_balance = $sender ? $sender['balance'] : 0;

        if ($sender_balance >= $amount) {
            // Update sender's balance (deduct the amount)
            ...
            // Log the transfer into the credit_transfers table
            $stmt = $conn->prepare("INSERT INTO credit_transfers (sender_id, receiver_id, amount) VALUES (?, ?, ?)");
            $stmt->execute([$user_id, $receiver_id, $amount]);
